...
def on_run_mainfile():
    ...
    dirpath, dirs, files = next(os.walk(inputdir.get()+"/in"))
    current = 0
    maxtime = 0
    passing = True
    ...
    for file in files:
        ...
        print(dirpath,"---",file)
        start = Time.time()
        with open(inputdir.get()+"/out/"+file, "w") as outf:
            ...
            with open(inputdir.get()+"/in/"+file, "r") as f:
            ...
            try:
                ...
                # outs = decode(outs,"shift_jis")
                # print("-"*100)
                # print(outs)
                # print("="*100)
                # print(decode(errs,"shift_jis"))
        ...
        temp = subprocess.Popen(
            'cd ./tools && cargo run --release --bin vis "'+inputdir.get()+'/in/'+file+'" "'+inputdir.get()+'/out/'+file,
            # "cargo --version",
        ...
        try:
            ...
            print("-"*100)
            print(decode(outs,"shift_jis").split("\n"))
            print("-"*100)
            print(decode(errs,"shift_jis").split("\n"))
            score = int(decode(outs,"shift_jis").split("\n")[0].split()[-1])
            total_score += score
        except subprocess.TimeoutExpired:
            temp.kill()
            outs, errs = temp.communicate()
...
col += 1
inputdir = StringVar(None, "D:/Competition Python/Atcoder/AHC 004/tools")
