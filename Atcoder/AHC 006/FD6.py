...
def search8(starts, ends):
    starts.sort(key = lambda x: ((ends[x["index"]]["dist"]+x["dist"]) / 2,x["coord"]))
    starts = starts[:50]
    starts.sort(key = lambda x: (x["angle"],x["dist"]))
    return best_path(starts, ends)
...
def main():
    ...
    searches = [search1, search2, search3, search4, search5, search6, search7, search8]
    ms = float("inf")
    indexes = []
    path = []
