...
def search8(starts, ends):
    ...
    while True:
        change = False
        for i in range(len(ans_coord)-4):
            a,b,c,d = ans_coord[i],ans_coord[i+1],ans_coord[i+2],ans_coord[i+3]
            if dist(a,b)+dist(c,d) > dist(a,c)+dist(b,d):
                ans_coord[i+1],ans_coord[i+2] = ans_coord[i+2],ans_coord[i+1]
                change = True
        if not change:
            break
    return ans_ind, ans_coord
...
def main():
    ...
    searches = [search1, search2, search3, search4, search5, search6, search7, search8, search9]
    ms = float("inf")
    indexes = []
    path = []
